...
from massspecgym.models.pfas import HalogenDetectorDreamsIonModeFFN
...
# removed adduct due to a str error (fixed here: converted to a numeric
# ion_mode class index before the generic tensor-cast loop, same pattern
# used below for the 'F' fluorine-presence flag)
class IonModeMassSpecDataset(MassSpecDataset):
    ...
    def __getitem__(
        ...
        item.update({
            k: metadata[k] for k in ["precursor_mz"] # removed adduct due to a str error (kept as-is; adduct is consumed above via ion_mode instead)
        })
...
#python3.11 train_PFAS_ion_mode_ffn_model.py
...
for i in range(0, num_iterations):
    # Load dataset
    dataset = IonModeMassSpecDataset(
    ...
    # Init model
    model = HalogenDetectorDreamsIonModeFFN(
        seed=seed,
    ...
    wandb_logger = WandbLogger(project=f'PFASDetection-IonModeFFN-MergedMassSpecNIST20_NISTNew')
